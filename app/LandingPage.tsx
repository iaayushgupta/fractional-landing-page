export default function LandingPage() {
  ...
  return (
    // Brown & beige theme (stone + amber)
    <div className="min-h-screen bg-gradient-to-b from-stone-50 via-amber-50/30 to-stone-50 text-stone-900">
      {/* Header */}
      <header className="sticky top-0 z-40 backdrop-blur bg-white/80 border-b border-stone-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="h-16 flex items-center justify-between">
            <a href="/" className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-xl bg-amber-500/10 ring-1 ring-amber-500/30 flex items-center justify-center">
            ...
            <nav className="hidden md:flex items-center gap-6 text-sm">
              {nav.map((n) => (
                <a key={n.href} href={n.href} className="hover:text-amber-700 transition">
                  {n.label}
                </a>
              ))}
            </nav>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10 opacity-40 bg-[radial-gradient(1200px_600px_at_50%_-100px,rgba(217,119,6,0.18),transparent)]" />
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#packages" className="rounded-xl bg-amber-600 hover:bg-amber-700 text-white px-5 py-3 font-semibold">
                View Packages
              </a>
            </div>
      ...
      {/* Packages */}
      <section id="packages" className="bg-stone-50 border-y border-stone-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-end justify-between">
            <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight">Productized packages</h2>
            <p className="text-sm text-stone-500">Fixed scope • Fixed timeline • Clear outcomes</p>
          </div>
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {packages.map((p) => (
              <div key={p.name} className="rounded-2xl bg-white border border-stone-200 p-6 flex flex-col">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold">{p.name}</h3>
                  <span className="text-xs rounded-full bg-amber-500/10 text-amber-800 px-2 py-1 border border-amber-500/20">
                    {p.tag}
                  </span>
                </div>
                <p className="mt-1 text-stone-600 text-sm">{p.price}</p>
                ...
                <div className="mt-6 pt-4 border-t border-stone-200">
                  <a
                    href="#contact"
                    className="inline-flex justify-center rounded-xl bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 font-semibold w-full"
                  >
                    {p.cta}
                  </a>
                </div>
      ...
      {/* Case Studies */}
      <section id="case-studies" className="bg-stone-50 border-y border-stone-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-end justify-between">
            <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight">Case studies</h2>
            <p className="text-sm text-stone-500">Brief, outcome-first, code-backed</p>
          </div>
          <div className="mt-8 grid gap-6 md:grid-cols-3">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 text-xs text-stone-700">
          {[
            "dbt",
            "Airflow",
            "Prefect",
            ...
            <div key={t} className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-center">
              {t}
            </div>
          ))}
        </div>
      ...
      {/* Team */}
      <section id="team" className="bg-stone-50 border-y border-stone-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
      ...
      <section id="contact" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <a
                ...
                className="rounded-xl bg-amber-600 hover:bg-amber-700 text-white px-5 py-3 font-semibold text-center"
              >
                Email us
              </a>
            </div>
        ...
        <p className="mt-4 text-xs text-stone-500">
          By contacting us you agree to our <a className="underline hover:text-amber-700" href="#">Privacy Policy</a>{" "}
          and <a className="underline hover:text-amber-700" href="#">Terms</a>.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {faqs.map((f) => (
            <div key={f.q} className="rounded-2xl bg-stone-50 border border-stone-200 p-5">
              <div className="font-semibold">{f.q}</div>
              <div className="mt-2 text-stone-700 text-sm">{f.a}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <footer className="border-t border-stone-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 text-sm text-stone-600 flex flex-col sm:flex-row items-center justify-between gap-4">
...
function Stat({ kpi, label }: { kpi: string; label: string }) {
  return (
    <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
      <div className="text-2xl font-semibold text-amber-800">{kpi}</div>
...
function SolutionCard({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="rounded-2xl border border-stone-200 bg-white p-5">
      ...
      <p className="mt-3 text-stone-700 text-sm">{desc}</p>
    </div>
  );
}
...
function Case({ title, metric, blurb }: { title: string; metric: string; blurb: string }) {
  return (
    <div className="rounded-2xl bg-white border border-stone-200 p-5">
      ...
      <div className="mt-2 text-xl font-semibold text-amber-800">{metric}</div>
      <p className="mt-2 text-stone-700 text-sm">{blurb}</p>
      <a href="/cases/d2c-forecast" className="mt-4 inline-flex items-center gap-2 text-amber-800 hover:underline text-sm">
        Read more{" "}
...
function Member({ name, role, url }: { name: string; role: string; url: string }) {
  return (
    <div className="rounded-2xl border border-stone-200 bg-white p-5 flex items-center gap-4">
      <div className="h-12 w-12 rounded-xl bg-stone-100 flex items-center justify-center text-stone-500 font-semibold">
        {name
          .split(" ")
          .map((w) => w[0])
          .join("")
      ...
      <div>
        ...
        <a href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-amber-800 hover:underline text-sm mt-1">
          LinkedIn
          <svg viewBox="0 0 24 24" className="h-4 w-4">
            <path d="M5 12h14M13 5l7 7-7 7" fill="none" stroke="currentColor" strokeWidth="2" />
          </svg>
...
function CheckIcon() {
  return (
    <svg viewBox="0 0 24 24" className="mt-1 h-4 w-4 flex-none text-amber-800">
